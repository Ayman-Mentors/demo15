...
class ResConfigSettings(models.TransientModel):
	...
	location_id = fields.Many2one('stock.location', string="Source Location", related='company_id.location_id', readonly=False)
	location_dest_id = fields.Many2one('stock.location', string="Destination Location", related='company_id.location_dest_id', readonly=False)
...
class StockReserve(models.TransientModel):
	...
	# if source_location and destination_location:
		...
		template_browse =  self.env.ref('sales_stock_reservation_app.reserved_stock_email_template')
		...
		if move_lines:
			...
			body_html += """</table><br />
							...
							Thanks & Regards.<br />
							from """+ str(self.sale_order.partner_id.company_id.name) or str(self.sale_order.company_id.name) + """.
						</p>
					</div> """
			email_cc_list = []
			...
			values = template_browse.generate_email(self.id, ['subject', 'body_html', 'email_from', 'email_to', 'partner_to', 'email_cc', 'reply_to', 'scheduled_date'])
